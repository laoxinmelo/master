...
//        }
        ...
        /**
         * 将数据库中电子烟的在线评论数据读取并保存到txt中
         */
        String sql = "select segmentNoSW from ecigar;";
        ResultSet resultSet = dbTool.query(sql);

        BufferedWriter bw = new BufferedWriter(new FileWriter("ecigar.txt"));


        while (resultSet.next()) {
            String segmentNoSW = resultSet.getString(1);
            System.out.println(segmentNoSW);
            bw.write(segmentNoSW + "\r\n");
        }
